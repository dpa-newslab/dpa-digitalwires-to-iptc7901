...
class AbstractIptcBuilder(ABC):
    ...
    def __init__(self, dw_model: DigitalwiresModel, line_length: int = 69):
    ...
    def wrap_line(self, paragraphs: list[str], double_newline: bool = False) -> str:
        """Wrap every line of every paragraph/table row/list item to self.line_length
        chars.

        :param self:
        :param paragraphs: a list of paragraphs that will be wrapped.
        :param double_newline: flag if each item in `paragraphs` should be double-
            wrapped. This is used in body content
